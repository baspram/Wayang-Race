...
public class Board {
	...
	public static final int NBLOCK = 42; 
	...
	//Method
	// Constructor
		public Board(int Nplayer, ArrayList<Player> Pl)
		{
			nplayer = Nplayer;
			tokenname = new String[Nplayer];
			for(int i=0; i<Nplayer; i++)
			{
				tokenname[i]=new String("token_" + Pl.get(i+1).getName().toLowerCase());
			}
			cell = new Cell[BOARDY][BOARDX];
			for(int i=0; i<BOARDY; i++)
				for(int j=0; j<BOARDX; j++)
					cell[i][j] = new Cell(tokenname,Nplayer);
				...
				for (int temp = 0; temp < nList.getLength(); temp++){
					...
					if (nNode.getNodeType() == Node.ELEMENT_NODE){
						...
						try
						{
							...
							for(int k=2 ; k<nplayer+2; k++)
								Game.boardPanel.add(cell[i][j].getTile()[k]);
								/*Game.boardPanel.add(cell[i][j].getTile()[3]);
								Game.boardPanel.add(cell[i][j].getTile()[4]);
								Game.boardPanel.add(cell[i][j].getTile()[5]);*/
							System.out.println("mana gettile??");
							Game.boardPanel.add(cell[i][j].getTile()[1]);
							Game.boardPanel.add(cell[i][j].getTile()[0]);
